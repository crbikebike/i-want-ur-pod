...
You are slice 0 of 8. Every command carries `--slice 0/8`:

    cd /home/hf/claude/i-want-ur-pod
    python3 -m admin.api.label vocab
    python3 -m admin.api.label next --limit 20 --slice 0/8
...
Before your first batch, run `ls w0*.json` and pick a prefix nobody has used yet (`w0c-b1.json`, `w0d-b1.json`, …). The scratchpad is shared with seven other agents and with every earlier wave on your own slice, so `w0-b1.json` is very likely taken. Overwriting a recorded file loses nothing, but overwriting a live one loses a sibling's batch.
