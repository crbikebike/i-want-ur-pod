...
You are slice 3 of 8. Every command carries `--slice 3/8`:
    ...
    python3 -m admin.api.label next --limit 20 --slice 3/8
...
Before your first batch, run `ls w3*.json` and pick a prefix nobody has used yet (`w3c-b1.json`, `w3d-b1.json`, …). The scratchpad is shared with seven other agents and with every earlier wave on your own slice, so `w3-b1.json` is very likely taken. Overwriting a recorded file loses nothing, but overwriting a live one loses a sibling's batch.
